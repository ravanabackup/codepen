/*
 * The design was completely made by @jackrugile in his other pen:
 * https://codepen.io/jackrugile/pen/AokpF
 * This is just a recreation of that pen
 *
 */
...
//==//==================//
    ...
    obstacleCount = ( ( width + height ) / 20 ) |0,
    obstacleRadius = 100,
    gravity = .1,
...
//==//===================================//
    ...
    step = function( ) {
      ...
      obstacles.map( function( obstacle ) { obstacle.step( ); } );
    },
    draw = function( ) {
      ...
      obstacles.map( function( obstacle ) { obstacle.draw( ); } );
    },
...
//==//====================================//
    ...
    Particle = function( ) { this.reset( ); },
    Obstacle = function( ) { this.reset( ); };
...
Particle.prototype = {
  ...
  reset: function( ) {
    ...
    this.color = 'hsla(hue, 80%, 50%, alp)'
      .replace( 'hue', ( frame + 360 * this.x / width) |0 );
  },
  ...
  step: function( ) {
    ...
    if( ( this.x < 0 || this.x > width ) && Math.random( ) < .4 ) this.vx *= -1;
  ...
  hit: function( obs ) {
    ...
    this.vx = ( this.x - obs.x ) * rand( .02, .03 );
    this.vy = ( this.y - obs.y ) * rand( .02, .03 );
  ...
  draw: function( ) {
    ...
    ctx.strokeStyle = this.color.replace( 'alp', .4 );
    ctx.beginPath( );
    ...
    if( this.shine ) {
      ...
      ctx.fillStyle = this.color.replace( 'alp', .05 );
      ctx.beginPath( );
