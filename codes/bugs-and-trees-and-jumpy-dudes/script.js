...
async function Draw(){
  if(!t){
    ...
    function loadAnimation(name, size, X, Y, Z, rl, pt, yw, speed=1) {
      ...
      fetch(`${rootURL}/${name}/fileList.json`).then(v => v.json()).then(data => {
        ...
        for(let i=0; i<+animation.fileList.fileCount; i++){
          let file = `${rootURL}/${name}/${animation.fileList.fileName}${i+(animation.hasLoop?animation.loopRangeStart:0)}.${animation.fileList.suffix}`
    ...
    drawAnimation = (ox,oy,oz,animation, scol='#8888', fcol='', lineWidth=2, glowing=true, overrideGlobalAlpha=1, speed=1, scale=1,normal=false) => {
      ...
      (l=animationFrameData[animation.idx])[Math.min(animation.curFrame|0,l.length-1)].map((v, i) => {
        x.beginPath()
        v.map(q=>{
          X = q[0] * scale
          Y = q[1] * scale
    ...
    let animRl            = 0
    let animPt            = 0
    let animYw            = Math.PI/2 + Math.PI
    let speed             = .75
    loadAnimation('tree_low_poly', animSize, animX,animY,animZ,animRl,animPt,animYw, speed)
    loadAnimation('back_flip_male', animSize, animX,animY,animZ,animRl,animPt,animYw, speed)
    loadAnimation('bug_lowpoly', animSize/3, animX,animY,animZ,animRl,animPt,animYw, speed)
    ...
    loadLandscape = () => {
      ...
      for(i=iTreec;i--;){
        ...
        landscape = [...landscape,
                       { X, Y, Z, 
                         animation:JSON.parse(JSON.stringify(animations[(Rn()*animationsCT)|0])),
                         rl: 0, pt: 0, yw: 0,
                         speed: 1 + Rn()*2, scale: (1+Rn()*3)/3 }]
      }
    }
  ...
  oX-=C(t)*20, oY=0, oZ+=50 + Math.min(50,Math.max(-50,(.3-C(t*2))*250))
  Rl=0, Pt=-.5, Yw=0
  ...
  if(showstars) ST.map(v=>{
    ...
    R(Rl,Pt,Yw,1)
    if(Z>0){
      if((x.globalAlpha = Math.min(1,(Z/1e6)**2))>.1){
        s = Math.min(1e3, 6e7/Z)
      ...
      for(let i=1400;i--;){
        ...
        if(d_<4e3 && alpha>.1){
          ...
          if(Z>0){
            ...
            x.fillStyle = '#4400ff14'
            x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
            s/=3
            x.fillStyle = '#0088ff20'
            x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
            s/=3
    ...
    landscape.map((tree,idx) => {
      ...
      if(d_<4e3){
        ...
        if((alpha = 1 / ((1+d_)**9/1e31)) > .1){
          col1 = ''
          col2 = `hsla(${360/landscape.length*idx/8+t*100},99%,50%,.4)`
          drawAnimation(tx, ty, tz, tree.animation, col1,col2, lineWidth=50, glowing=false, alpha, tree.speed, tree.scale, l[1])
        }
      }
    })
    x.globalAlpha = 1
