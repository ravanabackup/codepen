...
const smoke_colour = f0.addColor(uniforms, 'object_colour').name("Surface Colour");
const bg_colour = f0.addColor(uniforms, 'bg_colour').name("Background");
const f1 = gui.addFolder('Object');
const bump_strength = f1.add(uniforms, 'bump_strength', 0, 2).name("Bump Strength").step(0.01);
const shininess = f1.add(uniforms, 'shininess', 1, 200).name("Shininess").step(0.5);
const strength = f1.add(uniforms, 'strength', 0, 2).name("Specular Strength").step(0.01);
...
const fresnel_bias = f2.add(uniforms, 'fresnel_bias', 0, 2).name("Bias").step(0.01);
const fresnel_scale = f2.add(uniforms, 'fresnel_scale', 0, 20).name("Scale").step(0.01);
const fresnel_power = f2.add(uniforms, 'fresnel_power', 0, 20).name("Power").step(0.01);
// const metalness = gui.add(uniforms, 'metalness', 0, 1).name("Metalness").step(0.01);
const f3 = gui.addFolder('Environment');
const refl_power = f3.add(uniforms, 'refl_power', 0, 2).name("Power").step(0.01);
const refl_blur = f3.add(uniforms, 'refl_blur', 0, 10).name("Blur").step(0.01);
const f4 = gui.addFolder('Camera');
const cam_x = f4.add(uniforms, 'cam_x', -5, 5).name("Camera X").step(0.01);
const cam_y = f4.add(uniforms, 'cam_y', -5, 5).name("Camera Y").step(0.01);
const cam_z = f4.add(uniforms, 'cam_z', -5, 5).name("Camera Z").step(0.01);
const cam_fov = f4.add(uniforms, 'cam_fov', -2, 2).name("Camera FoV").step(0.01);
const cam_zoom = f4.add(uniforms, 'cam_zoom', -2, 2).name("Camera Zoom").step(0.01);
...
const curl_phase = f5.add(uniforms, 'spin_phase', -1, 1).name("Curl Phase").step(0.01);
