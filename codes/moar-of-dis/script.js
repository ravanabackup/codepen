...
async function Draw(){
  ...
  if(!t){
    ...
    iEv = 1, iEr = .5, iErv = .2,iEc = 3
    base_cylinder = Cylinder(1,4,1,h=4).map(v=>{
      v.map(q=>{
        q[1]-=h/2+iEr
      })
      return v
    })
    base_geo = geoSphere(0, 0, 0, iEc, iEr)[4]
    iEmitterc = 2
    G = 50
    emitters = Array(iEmitterc).fill().map(v=>{
      X = (Rn()-.5)*G
      Y = (Rn()-.5)*G
      Z = (Rn()-.5)*G
      vx = (Rn()-.5)*(.3+(iEv-.3))
      ...
      return [X, Y, Z, vx, vy, vz, rl, pt, yw, rlv, ptv, ywv, JSON.parse(JSON.stringify(base_geo))]
    })
    ...
    spawnP = () => {
      emitters.map(emitter=>{
        tx = emitter[0] + emitter[3]
        ty = emitter[1] + emitter[4]
        tz = emitter[2] + emitter[5]
        rl_ = emitter[6] + emitter[9]
        pt_ = emitter[7] + emitter[10]
        yw_ = emitter[8] + emitter[11]
        emitter[12].map(v=>{
          X = v[0]*8
          Y = v[1]*8
          Z = v[2]*8
          p1_ = Math.atan2(X,Z)
          p2_ = Math.acos(Y/(Math.hypot(X,Y,Z)+.001))
          d = Math.hypot(X,Y,Z)-h/2
          X = 0
          Y = -1
          Z = 0
          R(0,p2_,0)
          R(0,0,p1_)
          R(rl_,pt_,yw_)
          P = [...P,  [X*(iEr+h)+tx,Y*(iEr+h)+ty,Z*(iEr+h)+tz,X*iPv,Y*iPv,Z*iPv,1]]
        })
      })
    }
    bounding = subDividedCube(3,G*1.125*1.5,0).map(v=>{
      v.map(q=>{
        q[3] = q[4] = q[5] = 0
        q[6] = q[0]
        q[7] = q[1]
  ...
  oX=0, oY=0, oZ=Math.max(50,Math.min(150,(.3+C(t/2))*500))
  Rl=0, Pt=C(t)/4, Yw=S(t/4)*4
  ...
  bounding.map((v,i)=>{
    ...
    v.map(q=>{
      ...
      q[4] /= 1.025
      q[5] /= 1.025
      q[3] += (q[6]-q[0])/elasticity
      q[4] += (q[7]-q[1])/elasticity
      q[5] += (q[8]-q[2])/elasticity
      X1 = X = q[0] += q[3]
      Y1 = Y = q[1] += q[4]
      Z1 = Z = q[2] += q[5]
      disp.map(n=>{
        displacement = 1-((1-n[3])**2 + n[3]**2)
        X2 = n[0]
        Y2 = n[1]
        Z2 = n[2]
        d = (5+Math.hypot(a=X2-X,b=Y2-Y,e=Z2-Z))**2
        d2 = Math.hypot(X2,Y2,Z2)
        vx = X2/d2
        vy = Y2/d2
        vz = Z2/d2
        q[3] += vx*100*displacement/d
        q[4] += vy*100*displacement/d
        q[5] += vz*100*displacement/d
        d_ += Math.hypot(q[6]-q[0],q[7]-q[1],q[8]-q[2])/2
      })
      R(Rl,Pt,Yw,1)
      if(Z>0)x.lineTo(...Q())
    })
    alph = Math.max(0,(1-1/(1+d_**2/2000))*.2)
    col2 = alph>.05?`hsla(${d_**1.2+t*200},99%,${50+d_**2/500}%,${alph})`:(i%2?'':'#ffffff06')
    stroke('',col2,10,false)
  })
  ...
  P.map(v=>{
    ...
    bounding.map(k=>{
      ...
      ay /= k.length
      az /= k.length
      d = Math.hypot((X1+X2)/2-ax,(Y1+Y2)/2-ay,(Z1+Z2)/2-az)
      if(d<16 && (l=lineFaceI(X1,Y1,Z1,X2,Y2,Z2,k))){
        spawnDisplacement(...l[0])
        spawnSparks(...l[0])
        v[6]=0
      }
    })
  ...
  emitters.map(emitter=>{
    ...
    ty = emitter[1] += emitter[4]
    tz = emitter[2] += emitter[5]
    if(tx>G/2 || tx<-G/2) emitter[3]*=-1
    if(ty>G/2 || ty<-G/2) emitter[4]*=-1
    if(tz>G/2 || tz<-G/2) emitter[5]*=-1
    rl = emitter[6] += emitter[9]
    pt = emitter[7] += emitter[10]
    yw = emitter[8] += emitter[11]
    emitter[12].map(v=>{
      tx2 = v[0]
      ty2 = v[1]
      tz2 = v[2]
      p1 = Math.atan2(tx2,tz2)
      p2 = Math.acos(ty2/(Math.hypot(tx2,ty2,tz2)+.001))
      base_cylinder.map(q=>{
        x.beginPath()
        q.map(n=>{
          X = n[0]
