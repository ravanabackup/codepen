...
async function Draw(){
  ...
  if(!t){
    ...
    subs = 10
    base_shp = subDividedCube(16,2,.15).map(v=>{
      a = []
      v.map((q, j)=>{
        l = j
        X1 = v[l][0]
    ...
    ptv = .1*2
    genP = () => {
      base_P = subDividedOctahedron(2,0,0)
      P = [[0,0,0,
           rlv,ptv,ywv,
           geoSphere (0, 0, 0, iPc, 7)[4]
           ]]
    }
    genP()
  ...
  oX=0, oY=0, oZ=Math.min(32,Math.max(8,(.3-C(t/4))*150))
  Rl=0, Pt=-.2+S(t/3)*2, Yw = S(t/4)*4
    ...
    v.map(q=>{
      ...
      P.map((shp, idx) => {
        ...
        shp[6].map((bshp,idx2)=>{
          ...
          ty2 = ty1 + bshp[1]
          tz2 = tz1 + bshp[2]
          d = (4 + Math.hypot(a = tx2-q[0], b = ty2-q[1], e = tz2-q[2]))**9
          q[6] -= a * repulsion / d
          q[7] -= b * repulsion / d
          q[8] -= e * repulsion / d
        })
      ...
      if(Z>0){
        ...
        x.fillStyle = `hsl(${220+t*50},99%,50%)`
        x.globalAlpha = .02
        x.fillRect(l[0]-s/2,l[1]-s/2,s,s)
        s/=3
        x.globalAlpha = .15
      ...
      d_ += Math.hypot(q[3]-q[0], q[4]-q[1], q[5]-q[2])
    })
    d_ /= 1
    x.globalAlpha = 1
    col1 = col2 = `hsla(${180+d_**5 / 6e6+t*50},99%,${50+d_**5/8e7}%,${Math.min(.66,.05+d_**5/1e9)})`
