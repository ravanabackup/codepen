...
function process(vars){
	...
	if(vars.mbutton)loadMaze(vars, vars.sd);
	var p,d,t;
...
function createShape(x,y,z,sides){
	...
	var shape={},ls=1,x1,y1,z1,x2,y2,z2;
	shape.segs=[];
	shape.osegs=[];
	shape.x=x;
	shape.y=y;
...
function loadMaze(vars, sides){
	
	vars.shapes=[];
	var p,sd=sides,ls,x,y,z,x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,size=Math.floor(60*(1/sides)),spacing;
    console.log(sd)
	vars.sd=sd;
	switch(sd){
		case 4: spacing=0.70710678118654752440084436210485*2; break;
		case 6: spacing=0.86602540378443864676372317075294*2; break;
	}
...
function frame(vars) {
	...
	if(vars === undefined){
        ...
        sides.addEventListener("change", () => {
          loadMaze(vars, parseInt(document.getElementById("sides").value));
        })
