...
function init() {
  ...
  for (let i = 0; i < textureArraySize; i += 4) {
    ...
    dataPos.image.data[i] = r * .2 * Math.sin(a) * Math.cos(i);
    dataPos.image.data[i + 1] = r * .2 * Math.sin(a) * Math.sin(i);
    dataPos.image.data[i + 2] = r * .2 * Math.cos(a);
    dataPos.image.data[i + 3] = 1;

    dataVel.image.data[i] = dataPos.image.data[i] * 30.;
    dataVel.image.data[i + 1] = dataPos.image.data[i + 1] * 30.;
    dataVel.image.data[i + 2] = dataPos.image.data[i + 2] * 30.;
