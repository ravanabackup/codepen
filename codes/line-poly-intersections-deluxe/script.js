...
async function Draw(){
  ...
  if(!t){
    ...
    G=10
    shp = Tetrahedron(G, subs=3, sphereize = 8)
    iBc = 6
    iBls = 50
    iSpinnerRv = .05
    iSpinnerv = .1
    vx = iSpinnerv/2 + (Rn()-.5) * iSpinnerv
    vy = iSpinnerv/2 + (Rn()-.5) * iSpinnerv
    vz = iSpinnerv/2 + (Rn()-.5) * iSpinnerv
    rl=Rn()*Math.PI*2
    pt=Rn()*Math.PI*2
    yw=Rn()*Math.PI*2
    rlv = ptv = ywv = 0
    switch(Rn()*3|0){
      case 0: rlv = iSpinnerRv/2 +(Rn()-.5) * iSpinnerRv/2; break
      case 1: ptv = iSpinnerRv/2 +(Rn()-.5) * iSpinnerRv/2; break
      case 2: rlv = iSpinnerRv/2 +(Rn()-.5) * iSpinnerRv/2; break
    }
    spinner = [[0, 0, 0, vx, vy, vz, rl, pt, yw, rlv, ptv, ywv, geoSphere(0, 0, 0, iBc, iBls)[4],Tetrahedron(iBls/12,3,6)]]
    
    sparks = []
    iSparkv = .2
    iSparkNo = 5
    spawnSparks = (X,Y,Z) => {
      for(let i=iSparkNo;i--;){
        vx = (Rn()-.5)*iSparkv
        vy = (Rn()-.5)*iSparkv
        vz = (Rn()-.5)*iSparkv
  ...
  oX=0, oY=0, oZ=60
  Rl=S(t/2)/4, Pt=C(t/4)/3, Yw=S(t/4)*4
  ...
  shp.map(v=>{
    ...
    spinner.map(q=>{
      ...
      q[12].map(n=>{
        ...
        check = false
        for(N=20;N--;){
          if((d = Math.hypot(ax-(X2+X1)/20*N,ay-(Y2+Y1)/20*N,az-(Z2+Z1)/20*N))<12) check = true
        }
        if(check && (l=lineFaceI(X1,Y1,Z1,X2,Y2,Z2,v))){
          contacts = [...contacts, l[0]]
          X = l[0][0]
          Y = l[0][1]
          Z = l[0][2]
          spawnSparks(X,Y,Z)
  ...
  shp.map(v=>{
    ...
    contacts.map(n=>{
      ...
      Y2 = n[1]
      Z2 = n[2]
      d1 += 2/(1+Math.hypot(X2-ax,Y2-ay,Z2-az))**2
    })
    col2 = d1>.0125?`hsla(${0},99%,${50+d1*10**2}%,${Math.min(.5,d1*2)})`:''
    stroke('#f001',col2,8,true)
  })
