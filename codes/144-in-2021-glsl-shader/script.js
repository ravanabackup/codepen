...
class Canvas {
  ...
  init() {
    ...
    this.uniLocation[0] = this.gl.getUniformLocation(this.prg, 'time');
    this.uniLocation[1] = this.gl.getUniformLocation(this.prg, 'mouse');
    this.uniLocation[2] = this.gl.getUniformLocation(this.prg, 'resolution');
    // init vertex
    const position = [
      -1.0,  1.0,  0.0,
      1.0,  1.0,  0.0,
    ...
    const vPosArray = [vPosition];
    const vIndex = this.createIbo(index);
    
    this.setAttribute(vPosArray, this.attLocation, this.stride);
