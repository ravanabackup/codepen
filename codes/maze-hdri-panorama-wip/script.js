...
async function Draw(){
  if(!t){
    ...
    */
    ...
    //bg.loop = true
    //bg.muted = true
    //bg.defaultPlaybackRate = bg.playbackRate = 1//.25
    //bg.oncanplay = () => bg.play()
    //url = 'https://srmcgann.github.io/skyboxes/videos/starfield_looped.mp4'


    bg = new Image()
    url = 'https://srmcgann.github.io/skyboxes3/HDRI/space.jpg'
    ...
    showBG = true
    if(showBG) await fetch(url).then(res=>res.blob()).then(data=>bg.src = URL.createObjectURL(data))

    spawnMazeSection = (tx_, ty_, tz_, w, h, rl, pt, yw, size) => {
      let tx, ty, tz
      cl = w | 0
      rw = h | 0
      sp = size
    ...
    cl = 40
    rw = cl
    //rw = cl*1.2941176470588236|0
    size = 1.025
    maze = []
    //maze = spawnMazeSection(0, 1, 0, cl, rw, 0, 0, 0, size)
    minxd = 6e6
    maxxd = -6e6
    minyd = 6e6
    maxyd = -6e6
    maze.map(v=>{
      if(v[0]<minxd)minxd=v[0]
      if(v[0]>maxxd)maxxd=v[0]
      if(v[1]<minyd)minyd=v[1]
      if(v[1]>maxyd)maxyd=v[1]
    })
    createOpenings = false
    if(createOpenings) maze = maze.filter(v=>{
      return (v[0]>=minxd+size/2 || v[1]>=minyd+size) && (v[0]<maxxd-size*2 || v[1]<=maxyd-size*2)
    })
    ...
    mazeimg = new Image()
    url = 'https://srmcgann.github.io/skyboxes3/HDRI/maze_bold_dark.png'
    await fetch(url).then(res=>res.blob()).then(data=>mazeimg.src = URL.createObjectURL(data))
  }
  ...
  x('globalAlpha', .66)
  x('drawImage', mazeimg, 0, 0, c.width, c.height, 0,0,output.width*2,output.height*2)
  x('globalAlpha', 1)
