...
class Particle {
  ...
  draw(ctx) {
    if (this.opacity < 0.01 || this.size < 0.5) {
      return;
    }
...
const animate = () => {
  ...
  const ga = map(Math.cos(state.timer * 5), -1, 1, 0, 0.2);
  ...
  state.particles.forEach((particle, index) => {
    ...
    const nx = Math.abs(hw - particle.x) * mx / state.width;
    const ny = Math.abs(hh - particle.y) * my / state.height;

    const rz = Math.sin(state.timer) ** 3;

    const sin = Math.sin(state.timer);
    const cos = Math.cos(state.timer);
