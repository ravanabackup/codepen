...
async function Draw(){
  ...
  if(!t){
    ...
    bounding = Cylinder(5,8,18,64)
  }
  
  oX=0, oY=Math.min(24, Math.max(-10, (.3+S(t/2.5))*40)), oZ=Math.min(60, Math.max(30,(.3+C(t/2.5))*60))
  Rl=0, Pt=-S(t/2)/3, Yw=S(t/2)/1.5+t/4+Math.PI/2
  ...
  flashes.map((v,i) => {
    ...
    if(Z>0){
      l = Q()
      s = Math.min(1e4, 3e4/Z*v[3])
      x.drawImage(starImgs[1].img,l[0]-s/2,l[1]-s/2,s,s)
    }
    v[3]-=.2
  })

  P = P.filter(v=>v[6]>.1&&v[1]<(iBc/2) * sp + sp * 8)
  ...
  P.map((v,i) => {
    ...
    if(Z>0){
      l = Q()
      s = Math.min(1e4, 3e3/Z)//*v[6])
      x.drawImage(starImgs[2+(i%8)].img,l[0]-s/2/1.05,l[1]-s/2/1.05,s,s)
    }
    v[6] -= .0006
  })
